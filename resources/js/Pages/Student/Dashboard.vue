...
const deleteGradeProof = (gradeId) => {
    Swal.fire({ title: 'Hapus Bukti Rapor?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Hapus', cancelButtonText: 'Batal' }).then(r => {
        if (r.isConfirmed) router.delete(route('register.deleteGradeProof', gradeId), { preserveScroll: true, onSuccess: () => showSuccess('Bukti rapor dihapus.') });
    });
};
    ...
    <div class="min-h-screen bg-slate-50 font-sans antialiased text-slate-800 flex overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 h-screen transition-all duration-300">
            ...
            <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-4 lg:px-8 z-10 sticky top-0">
                ...
                <div class="flex items-center gap-4">
                    <!-- Dark Mode Toggle -->
                    <button @click="toggleDarkMode" class="p-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 transition-colors" title="Toggle Dark Mode">
            ...
            <div v-if="activeTab === 'dashboard'" class="max-w-5xl mx-auto space-y-6">
                ...
                <!-- Admin Notes -->
                <div v-if="registration?.admin_notes" class="bg-amber-50 border-l-4 border-amber-500 rounded-r-xl p-5 shadow-sm">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-bold text-amber-800 uppercase tracking-wider">Catatan dari Panitia</h3>
                            <div class="mt-1 text-sm text-amber-700">
                                <p class="whitespace-pre-wrap">{{ registration.admin_notes }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <form @submit.prevent="saveGrades('berkas')">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-5">
                        ...
                        <div class="mt-5 pt-5 border-t border-slate-100">
                            ...
                            <div v-if="props.registration?.grade?.proof_file_path" class="mt-3 flex items-center gap-3">
                                <a :href="'/storage/' + props.registration.grade.proof_file_path" target="_blank" class="px-4 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    Lihat Berkas
                                </a>
                                <button v-if="!isLocked" type="button" @click="deleteGradeProof(props.registration.grade.id)" class="px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                    Hapus File
                                </button>
                            </div>
                            <input v-else type="file" accept=".pdf,.jpg,.jpeg,.png" @change="e => gradeForm.proof_file = e.target.files[0]" class="block w-full text-sm text-slate-600 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-blue-50 file:text-blue-700 file:font-bold hover:file:bg-blue-100">
                        </div>
                    </div>
