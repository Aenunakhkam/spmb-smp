...
<head>
    <meta charset="UTF-8">
    <title>Laporan Resmi Pendaftaran SPMB - {{ date('Y') }}</title>
    <style>
        @page {
        ...
        table.summary-table { width: 100%; border-collapse: collapse; margin-bottom: 12px; font-size: 8pt; }
        table.summary-table td { border: 1px solid #000; padding: 4px 8px; vertical-align: middle; }
        .summary-header { background-color: #1B5E20; color: #fff; font-weight: bold; text-align: center; }
        ...
        .section-title { font-weight: bold; margin-bottom: 6px; margin-top: 10px; font-size: 9pt; background-color: #1B5E20; color: #fff; padding: 3px 8px; text-transform: uppercase; }
...
<body>
    ...
    <div class="judul-dokumen">
        LAPORAN RESMI KESELURUHAN PENDAFTAR PESERTA DIDIK BARU<br>
        TAHUN PELAJARAN {{ $settings['academic_year'] ?? date('Y').'/'.(date('Y')+1) }}
    </div>
    @endif

    <!-- REKAPITULASI RINGKAS -->
    <div class="section-title">I. REKAPITULASI STATISTIK PENDAFTAR</div>
    <table class="summary-table">
        <tr>
            <td class="summary-header" width="25%">TOTAL PENDAFTAR</td>
            <td class="summary-header" width="37.5%">BERDASARKAN STATUS</td>
            <td class="summary-header" width="37.5%">BERDASARKAN JENIS KELAMIN</td>
        </tr>
        <tr>
            <td style="text-align:center; font-size:14pt; font-weight:bold;">
                {{ $totalRegistrations }} <span style="font-size:9pt; font-weight:normal;">Siswa</span>
            </td>
            <td>
                @foreach($byStatus as $stat)
                    <b>{{ $statusLabels[$stat->status] ?? strtoupper($stat->status) }}:</b> {{ $stat->total }} siswa<br>
                @endforeach
            </td>
            <td>
                @foreach($byGender as $g)
                    <b>{{ $g['label'] }}:</b> {{ $g['total'] }} siswa<br>
                @endforeach
            </td>
        </tr>
    </table>
    ...
    <div class="section-title">II. DAFTAR RINCIAN PENDAFTAR CALON PESERTA DIDIK</div>
